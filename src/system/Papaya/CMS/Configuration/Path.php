 ...
 */
class Path extends \Papaya\Application\BaseObject implements StringCastable {
  ...
  public function get() {
    switch ($this->_basePath) {
      case self::PATH_TEMPLATES :
        $result = $this->papaya()->options->get(\Papaya\CMS\CMSConfiguration::PATH_TEMPLATES, '/');
      break;
      ...
      case self::PATH_UPLOAD :
        $result = $this->papaya()->options->get(\Papaya\CMS\CMSConfiguration::PATH_DATA, '/').
          $this->_path;
      break;
      default :
   ...
   * @return ThemeHandler
   */
  public function themeHandler(ThemeHandler $handler = NULL) {
    if (NULL !== $handler) {

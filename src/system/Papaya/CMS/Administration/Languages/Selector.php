  ...
  private function prepare() {
    ...
    if (NULL === $this->_current) {
      ...
      if ($id = $this->parameters()->get('lngsel[language_select]')) {
        ...
        ($id = $user->options->get(\Papaya\CMS\CMSConfiguration::CONTENT_LANGUAGE)) &&
        ($this->_current = $languages->getLanguage($id))
      ) {
        $application->session->values()->set([$this, 'CONTENT_LANGUAGE'], $this->_current->id);
      } elseif ($id = $application->options->get(\Papaya\CMS\CMSConfiguration::CONTENT_LANGUAGE)) {
        $this->_current = $languages->getLanguage($id);
      } elseif ($code = $application->options->get(\Papaya\CMS\CMSConfiguration::UI_LANGUAGE)) {

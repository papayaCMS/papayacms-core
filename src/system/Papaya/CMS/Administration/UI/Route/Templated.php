...
namespace Papaya\CMS\Administration\UI\Route {
  ...
  abstract class Templated implements Router\PathRoute, \Papaya\Application\Access {
    ...
    public function getOutput() {
      ...
      $template->parameters()->assign(
        [
          'PAGE_PROJECT' => $application->options->get(\Papaya\CMS\CMSConfiguration::PROJECT_TITLE, 'CMS Project'),
          'PAGE_REVISION' => \trim(\constant('PAPAYA_WEBSITE_REVISION')),
          'PAPAYA_DBG_DEVMODE' => $application->options->get(\Papaya\CMS\CMSConfiguration::DBG_DEVMODE, FALSE),
          ...
          'PAPAYA_UI_THEME' => preg_replace(
            '/.ini\s*$/i',
            '',
            $application->options->get(\Papaya\CMS\CMSConfiguration::UI_THEME, 'green')
          ),
          'PAPAYA_USE_RICHTEXT' => $application->administrationRichText->isActive(),
          'PAPAYA_RICHTEXT_CONTENT_CSS' =>
            $this->theme()->getURL(NULL, $application->options->get(\Papaya\CMS\CMSConfiguration::RICHTEXT_CONTENT_CSS)),
          'PAPAYA_RICHTEXT_TEMPLATES_FULL' =>
            $application->options->get(\Papaya\CMS\CMSConfiguration::RICHTEXT_TEMPLATES_FULL),
          'PAPAYA_RICHTEXT_TEMPLATES_SIMPLE' =>
            $application->options->get(\Papaya\CMS\CMSConfiguration::RICHTEXT_TEMPLATES_SIMPLE),
          'PAPAYA_RICHTEXT_LINK_TARGET' =>
            $application->options->get(\Papaya\CMS\CMSConfiguration::RICHTEXT_LINK_TARGET),
          'PAPAYA_RICHTEXT_BROWSER_SPELLCHECK' =>
            $application->options->get(\Papaya\CMS\CMSConfiguration::RICHTEXT_BROWSER_SPELLCHECK)
        ]
      );
    ...
    /**
     * @param \Papaya\CMS\Theme\Handler|null $themeHandler
     * @return \Papaya\CMS\Theme\Handler
     */
    public function theme(\Papaya\CMS\Theme\Handler $themeHandler = NULL) {
      if (NULL !== $themeHandler) {
        $this->_themeHandler = $themeHandler;
      } elseif (NULL === $this->_themeHandler) {
        $this->_themeHandler = new \Papaya\CMS\Theme\Handler();

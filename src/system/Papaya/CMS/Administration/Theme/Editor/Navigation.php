 ...
 */
class Navigation extends \Papaya\CMS\Administration\Page\Part {
  /**
   * @var UI\ListView
   */
  private $_listview;
   ...
   *
   * @param \Papaya\CMS\Content\Structure\Page $element
   *
   * @return UI\ListView\Item
   */
  private function createPageItem(\Papaya\CMS\Content\Structure\Page $element) {
    $item = new UI\ListView\Item('items-folder', (string)$element->title);
    $item->papaya($this->papaya());
    $item->indentation = 2;
    $item->reference->setParameters(

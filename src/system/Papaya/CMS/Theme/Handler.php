...
class Handler implements Application\Access {
  ...
  public function getURL($themeName = NULL, $fileName = NULL, $validateFileExists = TRUE) {
    ...
    if (Utility\Server\Protocol::isSecure()) {
      $baseURL = $options->get(\Papaya\CMS\CMSConfiguration::CDN_THEMES_SECURE, '');
    }
    if (empty($baseURL)) {
      $baseURL = $options->get(\Papaya\CMS\CMSConfiguration::CDN_THEMES, '');
    }
    if (empty($baseURL)) {
      $baseURL = $this
        ->papaya()
      ...
      $baseURL .= Utility\File\Path::cleanup(
        $options->get(\Papaya\CMS\CMSConfiguration::PATH_WEB).$options->get(\Papaya\CMS\CMSConfiguration::PATH_THEMES)
      );
    }
    if ('' === \trim($themeName)) {
  ...
  public function getLocalPath() {
    ...
    $path = $this
      ->papaya()
      ->options
      ->get(\Papaya\CMS\CMSConfiguration::PATH_THEMES);
  ...
  public function getTheme() {
    ...
    if (empty($theme)) {
      $theme = $this
        ->papaya()
        ->options
        ->get(\Papaya\CMS\CMSConfiguration::LAYOUT_THEME);
    }
    return $theme;
  }
  ...
  public function getThemeSkin() {
    ...
    if ($themeSkin <= 0) {
      $themeSkin = $this
        ->papaya()
        ->options
        ->get(\Papaya\CMS\CMSConfiguration::LAYOUT_THEME_SET, 0);
    }

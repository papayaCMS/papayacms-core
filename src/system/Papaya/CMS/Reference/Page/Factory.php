...
use Papaya\CMS\Reference as CMSReference;
...
class Factory implements Application\Access {
   ...
   *
   * @return CMSReference\Page
   */
  public function create(): CMSReference\Page {
    $pageReference = new CMSReference\Page();
    $pageReference->papaya($this->papaya());
   ...
   * @return CMSReference\Page
   */
  public function get($languageIdentifier, $pageId): CMSReference\Page {
   ...
   *
   * @param CMSReference\Page $reference
   *
   * @return CMSReference\Page
   */
  public function configure(CMSReference\Page $reference) {
    ...
    if (!($language && $language->isContent)) {
      ...
      if (!($language && $language->isContent)) {
        $language = $languages->getLanguage(
          $this->papaya()->options->get(\Papaya\CMS\CMSConfiguration::CONTENT_LANGUAGE, 1),
   ...
   *
   * @param CMSReference\Page $page
   *
   * @return bool
   */
  public function isStartPage(CMSReference\Page $page) {

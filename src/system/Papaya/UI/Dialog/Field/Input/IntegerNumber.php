 ...
 */
class IntegerNumber extends UI\Dialog\Field\Input {
  /**
   ...
   * @param int $minimum optional, default NULL
   * @param int $maximum optional, default NULL
   *
  ...
  public function __construct(
    ...
    $minimum = NULL !== $minimum ? (int)$minimum : NULL;
    $maximum = NULL !== $maximum ? (int)$maximum : NULL;
    if (NULL !== $minimum && NULL !== $maximum && $minimum > $maximum) {
      throw new \UnexpectedValueException(
        'Maximum value must be greater than or equal to minimum value.'
      );
    }
    parent::__construct($caption, $name, NULL !== $maximum ? strlen($maximum) : 1024, $default);
    $this->setMandatory($mandatory);
    $this->setFilter(
      new Filter\IntegerValue($minimum, $maximum)
    );
  }
}

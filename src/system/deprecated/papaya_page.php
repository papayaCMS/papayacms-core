...
class papaya_page extends base_object {
  ...
  /**
   * @var \Papaya\CMS\Content\Language
   */
  ...
  */
  function execute() {
    ...
    if (!$options->load()) {
      ...
      $controller->setTemplateFile($options->get(\Papaya\CMS\CMSConfiguration::ERRORDOCUMENT_503, ''));
      $controller->execute($application, $application->request, $application->response);
      $application->response->end();
    }

    $baseSessionDomain = $options->get(\Papaya\CMS\CMSConfiguration::SESSION_DOMAIN, '');
    ...
    if ($options->get(\Papaya\CMS\CMSConfiguration::CONTENT_LANGUAGE_COOKIE, FALSE) &&
    ...
    if (
      isset($_GET['exit']) &&
      $options->get(\Papaya\CMS\CMSConfiguration::EXIT_REDIRECT_ENABLE, FALSE)
    ) {
    ...
    if ($options->get(\Papaya\CMS\CMSConfiguration::DEFAULT_HOST, '') != '' &&
        $options->get(\Papaya\CMS\CMSConfiguration::DEFAULT_HOST_ACTION, 0) > 0 &&
        strtolower($_SERVER['HTTP_HOST']) != strtolower($options->get(\Papaya\CMS\CMSConfiguration::DEFAULT_HOST))) {
      $this->doRedirectToPath(
  ...
  function protectedRedirect($code, $targetURL) {
    if ($this->papaya()->options->get(\Papaya\CMS\CMSConfiguration::REDIRECT_PROTECTION, FALSE)) {
  ...
  function initializeParams($sessionParamName = NULL) {
    if (isset($_REQUEST['tt']['page_id']) && $_REQUEST['tt']['page_id'] > 0) {
      ...
      if (TRUE === $aliasPage) {
        ...
        $this->topicId = $this->papaya()->options->get(\Papaya\CMS\CMSConfiguration::PAGEID_DEFAULT, 0);
      } else {
  ...
  */
  function checkAlias() {
    ...
    $publicFilesPath = $options->get(\Papaya\CMS\CMSConfiguration::PATH_PUBLICFILES, '');
    if (preg_match('(^/?([\\("]|\\\\"))', $requestURI)) {
      ...
      */
      ...
      $webFile = 'papaya-themes/'.$options->get(\Papaya\CMS\CMSConfiguration::LAYOUT_THEME, 'default').'/favicon.ico';
  ...
  */
  function setRequestFromAlias($alias) {
    ...
    $parameterGroupSeparator = $application
      ->options
      ->get(\Papaya\CMS\CMSConfiguration::URL_LEVEL_SEPARATOR);
    $request = $application->request;
  ...
  */
  function queryStringToArray($queryString) {
    $query = new Request\Parameters\QueryString(
      $this->papaya()->options->get(\Papaya\CMS\CMSConfiguration::URL_LEVEL_SEPARATOR, '')
    );
  ...
  function startSession() {
    ...
    if ($this->isPreview()) {
      $startSession = \Papaya\Session::ACTIVATION_DYNAMIC;
              ...
              $this->papaya()->options->get(\Papaya\CMS\CMSConfiguration::SESSION_START, FALSE)) {
    ...
    if (\Papaya\Utility\Server\Agent::isRobot()) {
      ...
      if ($this->papaya()->options->get(\Papaya\CMS\CMSConfiguration::SESSION_START, FALSE) &&
          !$this->papaya()->options->get(\Papaya\CMS\CMSConfiguration::DB_CONNECT_PERSISTENT, FALSE)) {
        $this->output->databaseClose();
      }
      $fallback = $this->papaya()->options->get(\Papaya\CMS\CMSConfiguration::SESSION_ID_FALLBACK, 'rewrite');
      switch ($fallback) {
      case 'get' :
  ...
  */
  function useCache() {
    ...
    return (
      !$this->isPreview() &&
      $method == 'GET' &&
      $this->papaya()->options->get(\Papaya\CMS\CMSConfiguration::CACHE_OUTPUT, FALSE) &&
      $this->papaya()->options->get(\Papaya\CMS\CMSConfiguration::CACHE_TIME_OUTPUT, 0) > 0 &&
  ...
  */
  function initPageMode() {
    ...
    case 'xml':
      ...
      break;
    case 'php':
      $defaultExtension = $this->papaya()->options->get(\Papaya\CMS\CMSConfiguration::URL_EXTENSION, 'html');
      $response = new \Papaya\Response\Redirect(
    ...
    if (
      ...
      if ($this->isPreview()) {
        $pageStatus = new \Papaya\CMS\Content\Page\Status();
      } else {
        $pageStatus = new \Papaya\CMS\Content\Page\Publication\Status();
      }
      $pageStatus->load($this->topicId);
      if ($pageStatus->sessionMode == 0) {
        $this->allowSession = $this->papaya()->options->get(
          \Papaya\CMS\CMSConfiguration::SESSION_ACTIVATION, \Papaya\Session::ACTIVATION_ALWAYS
        );
      } else {
        $this->allowSession = $pageStatus->sessionMode;
      }
      if ($this->allowSession != \Papaya\Session::ACTIVATION_NEVER) {
        if ($this->papaya()->options->get(\Papaya\CMS\CMSConfiguration::SESSION_CACHE) == 'nocache') {
  ...
  */
  function getStatus() {
    ...
    foreach ($paths as $name => $path) {
      ...
      if (!($status[$name] = $directory->isWritable())) {
        $allStatus = FALSE;
      }
    }
          ...
          if ($viewId > 0) {
            if ($this->filter = $this->output->getFilter($viewId)) {
              ...
              if ($this->filter->checkConfiguration()) {
                ...
                if ($this->acceptGzip  &&
                    $this->papaya()->options->get(\Papaya\CMS\CMSConfiguration::COMPRESS_OUTPUT, FALSE) &&
                    $this->canUseGzip()) {
  ...
  */
  ...
  function generatePage($filterParams = NULL, $outputContent = TRUE, $allowRedirect = TRUE) {
    ...
    if ($outputContent) {
      ...
      $url = str_replace('\\', '/', $serverURL.$this->papaya()->options->get(\Papaya\CMS\CMSConfiguration::PATH_WEB, '/'));
      ...
      $this->layout->parameters()->set(
        ...
        (string)$this->papaya()->references->byString(
          $this->papaya()->options->get(\Papaya\CMS\CMSConfiguration::PAGEID_DEFAULT, 1)
        )
      );
      ...
      $this->layout->parameters()->set('PAGE_THEME_SET', $themeHandler->getThemeSkin());
      ...
      $this->layout->parameters()->set(
        'PAGE_WEB_PATH', $this->papaya()->options->get(\Papaya\CMS\CMSConfiguration::PATH_WEB, '/')
      );
      $this->layout->parameters()->set(
        'PAGE_LANGUAGE',
      ...
      $this->layout->parameters()->set(
        ...
        $this->papaya()->options->get(\Papaya\CMS\CMSConfiguration::DEBUG_LANGUAGE_PHRASES, FALSE)
      );
    ...
    if (!empty($mediaId)) {
      ...
      if (preg_match($mediaFilePattern, $mediaId, $matches)) {
        ...
        if ($file = $this->checkMediaPerm($matches[2])) {
          $path = '';
          $depth = $this->papaya()->options->get(\Papaya\CMS\CMSConfiguration::MEDIADB_SUBDIRECTORIES, 1);
          for ($i = 0; $i < $depth; $i++) {
            $path .= $matches[1][$i].'/';
          }
          $fileName = $this->papaya()->options->get(\Papaya\CMS\CMSConfiguration::PATH_THUMBFILES).$path.$matches[1];
          if (isset($matches[9])) {
            $fileName .= $matches[9];
  ...
  */
  private function _storageSetPublic($storageGroup, $storageId, $mimeType) {
    ...
    $storage = \Papaya\Media\Storage::getService(
      $options->get(\Papaya\CMS\CMSConfiguration::MEDIA_STORAGE_SERVICE),
      $options
    );
  ...
  */
  ...
  function getError($error, $errorString, $errorCode = NULL, $verbose = FALSE) {
    if (!$verbose) {
      $this->mode = $this->papaya()->options->get(\Papaya\CMS\CMSConfiguration::URL_EXTENSION, 'html');
      $this->topic = $this->createPage();
      ...
      case 404:
        $errorStatus = 404;
        $this->topic->loadOutput(
          $this->papaya()->options->get(\Papaya\CMS\CMSConfiguration::PAGEID_ERROR_404, 0), $lngIdent
        );
        break;
      case 403:
        $errorStatus = 403;
        $this->topic->loadOutput(
          $this->papaya()->options->get(\Papaya\CMS\CMSConfiguration::PAGEID_ERROR_403, 0), $lngIdent
        );
        break;
      default:
        $errorStatus = 500;
        $this->topic->loadOutput(
          $this->papaya()->options->get(\Papaya\CMS\CMSConfiguration::PAGEID_ERROR_500, 0), $lngIdent
        );
      }
      $this->error = array(
        'status' => $errorStatus,
      ...
      if ((empty($_GET['redirect']) || $_GET['redirect'] != $errorStatus) &&
        ...
        $targetURL = $this->getAbsoluteURL(
          $this->papaya()->options->get(\Papaya\CMS\CMSConfiguration::PATH_WEB, '/'),
          $this->getBaseLink()
        );
        $targetURL .=
  ...
  */
  function getRedirect($status, $url) {
    ...
    case 302:
      $this->topic->loadOutput(
        $this->papaya()->options->get(\Papaya\CMS\CMSConfiguration::PAGEID_STATUS_302, 0),
        $lngIdent
      );
      break;
    case 301:
    default :
      $status = 301;
      $this->topic->loadOutput(
        $this->papaya()->options->get(\Papaya\CMS\CMSConfiguration::PAGEID_STATUS_301, 0),
        $lngIdent
      );
      break;
    }
  ...
  */
  ...
  function sendHeader($headerStr, $replace = TRUE) {
    ...
    if (isset($headersSent) && $headersSent) {
      ...
      if ($this->papaya()->options->get(\Papaya\CMS\CMSConfiguration::DISABLE_XHEADERS, FALSE) &&

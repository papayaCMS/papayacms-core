...
class base_object extends BaseObject implements \Papaya\Request\Parameters\Access {
  ...
  */
  function _gt($phrase, $module = NULL) {
    if (
      trim($phrase) !== '' &&
      isset($this->papaya()->administrationPhrases)
    ) {
  ...
  */
  function _gtfile($fileName) {
    ...
    $fileName = \Papaya\Utility\File\Path::cleanup(
      ...
      $this->papaya()->options->get(\Papaya\CMS\CMSConfiguration::PATHWEB_ADMIN, '/papaya/').
      '/data/'.$lng.'/'
    ).$fileName;
  ...
  */
  ...
  public function parseRequestURI($url = NULL) {
    ...
    if ($this->papaya()->request->isAdministration) {
      ...
      $ext = $this->papaya()->options->get(\Papaya\CMS\CMSConfiguration::URL_EXTENSION, 'html');
    }
  ...
  */
  function escapeForFilename($str, $default = 'index', $language = NULL) {
    $str = \Papaya\Utility\File::normalizeName(
      $str,
      $this->papaya()->options->get(\Papaya\CMS\CMSConfiguration::URL_NAMELENGTH, 50),
      $language
    );
  ...
  */
  function getWebMediaLink($mid, $mode = 'media', $text = '', $ext = '') {
    $application = $this->papaya();
    $request = $application->request;
    $thumbsFileType = $this->papaya()->options->get(\Papaya\CMS\CMSConfiguration::THUMBS_FILETYPE, IMAGETYPE_PNG);
    if (in_array($mode, array('thumb', 'thumbs', 'thumbnail'))) {
      $reference = new \Papaya\CMS\Reference\Thumbnail();
      ...
      $reference = new \Papaya\CMS\Reference\Media();
      $reference->papaya($this->papaya());
  ...
  function getAbsoluteURL($url, $text = '', $sid = TRUE, $protocol = NULL) {
    ...
    if (!($hostPort)) {
      $hostPort = $this->papaya()->options->get(\Papaya\CMS\CMSConfiguration::DEFAULT_HOST, 'localhost');
    }
    ...
    $pathWeb = $this->papaya()->options->get(\Papaya\CMS\CMSConfiguration::PATH_WEB, '/');
    if ($url == '/') {
    ...
    $pathAdmin = $this->papaya()->options->get(\Papaya\CMS\CMSConfiguration::PATH_ADMIN, '/');
    $href = preg_replace(

...
class RecordsTest extends \Papaya\TestCase {
  ...
  /**
   * @covers \Papaya\CMS\Plugin\Filter\Content\Records
   */
  public function testRecordsGetAfterSet() {
    $filterGroup = new Records($this->getPageFixture());
    $filterGroup->records($records = $this->createMock(\Papaya\CMS\Content\View\Configurations::class));
  ...
  public function testRecordsImplicitCreate() {
    ...
    $this->assertInstanceOf(
      \Papaya\CMS\Content\View\Configurations::class,
      $filterGroup->records()
    );
  }
  ...
  public function testIteratorFetchesPlugins() {
    $plugins = $this->createMock(\Papaya\CMS\Plugin\Loader::class);
    $plugins
      ->expects($this->once())
      ->method('get')
      ->with('guid', $this->isInstanceOf(\Papaya\CMS\Output\Page::class), 'options')
      ->will($this->returnValue($this->createMock(\Papaya\CMS\Plugin\Filter\Content::class)));

    $records = $this->createMock(\Papaya\CMS\Content\View\Configurations::class);
    $records
      ->expects($this->once())
      ->method('getIterator')
      ->will(
